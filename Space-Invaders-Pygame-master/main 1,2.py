...
jugador_y = 540
jugador_x_cambio = 0
jugador_velocidad = 0.5

# Enemigos
imagen_enemigo_original = pygame.image.load('enemigos.gif')
...
# Crear enemigos en una grilla de 5 filas por 11 columnas
for fila in range(filas):
    for columna in range(columnas):
        enemigos.append({
            "x": inicio_x + columna * espaciado_x,
            "y": inicio_y + fila * espaciado_y,
            "fila": fila,  # Guardamos la fila del enemigo
            "vivo": True
        })
...
balas_enemigas = []
tiempo_ultimo_disparo = 0  # Controlar frecuencia de disparos
...
# Mostrar texto de "fin del juego"
...
# Dibujar muros con su respectiva imagen según salud
def dibujar_muros():
    for muro in muros:
        if muro["salud"] > 0:
